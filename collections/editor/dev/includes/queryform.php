			...
			<div class="fieldGroupDiv">
				<div class="bottom-breathing-room-relative">
					<button type="button" class="icon-button no-margin-left" onclick="copyQueryLink(event)" title="<?php echo (isset($LANG['COPY_SEARCH'])?$LANG['COPY_SEARCH']:'Copy Search As Link'); ?>">
						<img src="<?= $CLIENT_ROOT ?>/images/dl2.png" srcset="<?= $CLIENT_ROOT ?>/images/link.svg" class="svg-icon" style="width:15px; height:15px" alt="Link icon. Copies the search terms as a link." />
					</button>
					<?php
					if(!$crowdSourceMode){
						$qryStr = '';
						...
						if($qryStr){
							?>
							<a href="<?= $CLIENT_ROOT ?>/collections/reports/labelmanager.php?collid=<?php echo $collId . $qryStr; ?>" target="_blank">
								<button type="button" class="icon-button" title="<?php echo $LANG['GO_LABEL_PRINT']; ?>">
									<img src="<?= $CLIENT_ROOT ?>/images/list.png" style="width:15px; height:15px" />
								</button>
							</a>
							<?php
						}
