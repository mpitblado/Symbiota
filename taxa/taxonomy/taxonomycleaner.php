		...
		<div id="innertext">
			...
			if($SYMB_UID){
								...
								if(array_key_exists('col',$snArr)){
									...
									if(array_key_exists('soundex',$snArr)){
										foreach($snArr['soundex'] as $t => $s){
											echo '<div style="margin-left:15px;font-weight:bold;">';
											echo $s;
											echo ' <a href="" title="' . htmlspecialchars((isset($LANG['REMAP_TO_NAME'])?$LANG['REMAP_TO_NAME']:'Remap to this name'), ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE) . '...">==>></a>';
											echo '</div>';
										}
									}
								}
				...
				<div style="font-weight:bold;">
					<?php echo (isset($LANG['PLEASE'])?$LANG['PLEASE']:'Please')."<a href='../../profile/index.php?refurl=" . htmlspecialchars($CLIENT_ROOT, ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE) . "/taxa/taxonomy/taxonomycleaner.php?collid=" . htmlspecialchars($collId, ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE) . ">" . htmlspecialchars((isset($LANG['LOGIN'])?$LANG['LOGIN']:'log in'), ENT_COMPAT | ENT_HTML401 | ENT_SUBSTITUTE) . "</a>!" ?>
				</div>
				<?php
			}
			?>
