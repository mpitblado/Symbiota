function getObservationSvg(
  opts = { color: "#7A8BE7", size: 24, className: "" }
) {
...
>
<polygon class="${opts.className}" points="${half},0 0,${opts.size} ${
      opts.size
    },${opts.size}" style="fill:${opts.color};stroke:black;stroke-width:3" />
</svg>`,
    className: "",
    observation: true,
...
class LeafletMap {
  ...
  enableDrawing(drawOptions = this.DEFAULT_DRAW_OPTIONS, onDrawChange) {
    ...
    const setDrawColor = (drawOption) => {
      if (drawOptions[drawOption] === false) return;
        ...
        function (e) {
          ...
          if (keys.length > 0) {
            ...
            this.shapes = this.shapes.map((s) =>
              s.layer._leaflet_id === edited_shape.layer._leaflet_id
                ? edited_shape
                : s
            );
          }
      ...
      this.mapLayer.on(
        ...
        function (e) {
          ...
          this.shapes = this.shapes.filter(
            (s) => !ids.includes(`${s.layer._leaflet_id}`)
          );
          this.activeShape = null;
          if (onDrawChange) onDrawChange(this.activeShape);
        }.bind(this)
...
function getShapeCoords(layerType, layer) {
  ...
  switch (layerType) {
    case "polygon":
      ...
      let polygon = latlngs.map(
        (coord) => `${coord[0].toFixed(SIG_FIGS)} ${coord[1].toFixed(SIG_FIGS)}`
      );
