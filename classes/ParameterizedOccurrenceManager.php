...
class ParameterizedOccurrenceManager extends OccurrenceTaxaManager {
    ...
    protected function handleTargetClidSql($sqlWhere){
        if(!$this->voucherManager){
			...
			if(array_key_exists('mode', $_REQUEST) && $_REQUEST['mode'] == 'voucher'){
				...
				if($clOccidArr) $sqlWhere .= 'AND (o.occid NOT IN('.implode(',',$clOccidArr).')) '; // @TODO
			}
            return $sqlWhere;
    }

    protected function handleClidSql($sqlWhere){
        if(isset($this->searchTermArr["cltype"]) && $this->searchTermArr["cltype"] == 'all'){
            $sqlWhere .= 'AND (cl.clid IN('.$this->searchTermArr['clid'].')) '; // @TODO
        }
        else{
            $sqlWhere .= 'AND (ctl.clid IN('.$this->searchTermArr['clid'].')) '; // @TODO
        }
    ...
    protected function handleDbSql($sqlWhere){
        $dbSearchTerm = $this->cleanInStr($this->searchTermArr['db']);
		$sqlRet = "";
		...
		if($dbSearchTerm != 'all'){
			...
			elseif($dbSearchTerm == 'allobs'){
				...
				$dbStr = "o.collid IN(" . (is_array($dbArr)? implode(',', $dbArr): $dbArr) . ")"; // @TODO
				$sqlRet .= 'AND ('.$dbStr.') '; // @TODO
			}
		}
		return $sqlWhere . $sqlRet;
    }

    protected function handleDatasetIdSql($sqlWhere){
        $sqlWhere .= 'AND (ds.datasetid IN('.$this->searchTermArr['datasetid'].')) '; // @TODO
        $this->displaySearchArr[] = $this->LANG['DATASETS'] . ': ' . $this->getDatasetTitle($this->searchTermArr['datasetid']);
    }

    protected function handleTaxonSql($sqlWhere){
        $sqlWhere .= $this->getTaxonWhereFrag(); // @TODO
        return $sqlWhere;
    }

    protected function handleCountrySql($sqlWhere){
        ...
        $sqlWhere .= 'AND ('.implode(' OR ',$tempArr).') '; // @TODO
        $this->displaySearchArr[] = implode(' ' .  $this->LANG['OR'] . ' ', $countryArr);
        return $sqlWhere;
    }

    protected function handleStateSql($sqlWhere){
    ...
    protected function handleCountySql($sqlWhere){
    ...
    protected function handleLocalSql($sqlWhere){
        ...
        $sqlWhere .= 'AND ('.implode(' OR ',$tempSqlArr).') '; // @TODO
        if($tempTermArr) $this->displaySearchArr[] = implode(' ' .  $this->LANG['OR'] . ' ', $tempTermArr);
        return $sqlWhere;
    }

    protected function handleElevLowSql($sqlWhere){
        $elevlow = -200;
        $elevhigh = 9000;
        ...
        $sqlWhere .= 'AND (( minimumElevationInMeters >= '.$elevlow.' AND maximumElevationInMeters <= '.$elevhigh.' ) OR ' .
            '( maximumElevationInMeters is null AND minimumElevationInMeters >= '.$elevlow.' AND minimumElevationInMeters <= '.$elevhigh.' )) '; // @TODO
        $this->displaySearchArr[] = $this->LANG['ELEV'] . ': ' . $elevlow . ($elevhigh ? ' - ' . $elevhigh : '');
        return $sqlWhere;
    }

    protected function handleLlboundSql($sqlWhere){
        $llboundArr = explode(";",$this->searchTermArr["llbound"]);
			if(count($llboundArr) == 4){
                ...
                $lLng = $llboundArr[2];
                $rLng = $llboundArr[3];
                $sqlWhere .= 'AND (ST_Within(p.point,GeomFromText("POLYGON(('.$uLat.' '.$rLng.','.$bLat.' '.$rLng.','.$bLat.' '.$lLng.','.$uLat.' '.$lLng.','.$uLat.' '.$rLng.'))"))) '; // @TODO
                $this->displaySearchArr[] = $this->LANG['LAT'] . ': ' . $llboundArr[1] . ' - ' . $llboundArr[0] . ' ' . $this->LANG['LONG'] . ': '.$llboundArr[2].' - '.$llboundArr[3];
			}
        return $sqlWhere;
    }

    protected function handleLlpointSql($sqlWhere){
        ...
        if(count($pointArr) == 4){
            ...
            $sqlWhere .= 'AND o.occid IN(SELECT occid FROM omoccurrences WHERE (DecimalLatitude BETWEEN '.$lat1.' AND '.$lat2.') AND (DecimalLongitude BETWEEN '.$long1.' AND '.$long2.')) '; // @TODO
            ...
            $sqlWhere .= 'AND (( 3959 * acos( cos( radians('.$lat.') ) * cos( radians( o.DecimalLatitude ) ) * cos( radians( o.DecimalLongitude )'.
                    ' - radians('.$lng.') ) + sin( radians('.$lat.') ) * sin(radians(o.DecimalLatitude)) ) ) < '.$radius.') '; // @TODO
        }
        $this->displaySearchArr[] = $pointArr[0] . ' ' . $pointArr[1] . ' +- ' . $pointArr[2] . $pointArr[3];
        return $sqlWhere;
    }

    protected function handleFootprintwktSql($sqlWhere){
        $sqlWhere .= 'AND (ST_Within(p.point,GeomFromText("'.$this->searchTermArr['footprintwkt'].'"))) '; // @TODO
        $this->displaySearchArr[] = $this->LANG['POLYGON_SEARCH'];
        return $sqlWhere;
    }

    protected function handleCollectorSql($sqlWhere){
        ...
        if($tempCollSqlArr) $sqlWhere .= 'AND ('.implode(' OR ',$tempCollSqlArr).') '; // @TODO
        $this->displaySearchArr[] = implode(' ' .  $this->LANG['OR'] . ' ', $tempCollTextArr);
        return $sqlWhere;
    }

    protected function handleCollnumSql($sqlWhere){
        ...
        if($rnWhere){
            $sqlWhere .= "AND (".substr($rnWhere,3).") "; // @TODO
            $this->displaySearchArr[] = implode(", ",$collNumArr);
        }
        return $sqlWhere;
    }

    protected function handleEventdate1Sql($sqlWhere){
        $dateArr = array();
        ...
        elseif($eDate1 = $this->cleanInStr($this->formatDate($dateArr[0]))){
            ...
            if($eDate2){
                ...
                $sqlWhere .= 'AND (o.eventdate BETWEEN "'.$eDate1.'" AND "'.$eDate2.'") '; // @TODO
            }
            else{
                if(substr($eDate1,-5) == '00-00'){
                    $sqlWhere .= 'AND (o.eventdate LIKE "'.substr($eDate1,0,5).'%") '; // @TODO
                }
                elseif(substr($eDate1,-2) == '00'){
                    $sqlWhere .= 'AND (o.eventdate LIKE "'.substr($eDate1,0,8).'%") '; // @TODO
                }
                else{
                    $sqlWhere .= 'AND (o.eventdate = "'.$eDate1.'") '; // @TODO
                }
            }
    ...
    protected function handleCatnumSql($sqlWhere){
        ...
        $sqlWhere .= 'AND ('.substr($catWhere,3).') '; // @TODO
        $this->displaySearchArr[] = $this->searchTermArr['catnum'];
        return $sqlWhere;
    }

    protected function handleMaterialsampletypeSql($sqlWhere){
    ...
    protected function handleAttrSql($sqlWhere){
        ...
        $sqlWhere .= 'AND (o.occid IN(SELECT occid FROM tmattributes WHERE stateid IN(' . $this->searchTermArr['attr'] . '))) '; // @TODO
        return $sqlWhere;
    }
	...
	protected function setSqlWhere(){
		...
		if(array_key_exists('attr',$this->searchTermArr)){
            $sqlWhere = $this->handleAttrSql($sqlWhere);
		}
