...
class OccurrenceMaintenance {
	...
	public function generalOccurrenceCleaning(){
		...
		$sql = 'SELECT o.occid FROM omoccurrences o INNER JOIN media i ON o.occid = i.occid WHERE (i.tid IS NULL) AND (o.tidinterpreted IS NOT NULL) ';
	...
	private function batchUpdateImageTid($occidArr){
		$status = false;
		if($occidArr){
			$sql = 'UPDATE omoccurrences o INNER JOIN media i ON o.occid = i.occid SET i.tid = o.tidinterpreted WHERE o.occid IN('.implode(',',$occidArr).')';
			if($this->conn->query($sql)){
				$status = true;
			}
			else{
			...
			foreach($collArr as $collid){
				if(is_numeric($collid)){
					...
					$sql = 'SELECT count(DISTINCT o.occid) as imgspeccnt, count(DISTINCT i.imgid) AS imgcnt
						FROM omoccurrences o INNER JOIN media i ON o.occid = i.occid
