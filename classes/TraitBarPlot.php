  ...
  private function tickSVG() {   // tick lines
    ...
    if(isset($this->ShowScale) && $this->ShowScale) {
      if(isset($this->TickNumber) && $this->TickNumber) {
        ...
        $svgStr .= '<defs>'.PHP_EOL.'<pattern id="'.$barPlotId.'" width="'.$tickInterval.'" height="'.$tickInterval.'" patternUnits="userSpaceOnUse">'.PHP_EOL;
        $svgStr .= '<rect class="' . $this->PlotClass . 'GridBackground" width="'.$tickInterval.'" height="'.$tickInterval.'" />'.PHP_EOL;
        $svgStr .= '<path class="' . $this->PlotClass . 'GridLine" d="M '.$tickInterval.' 0 L 0 0 0 '.$tickInterval.'" fill="none" />'.PHP_EOL.'</pattern>'.PHP_EOL.'</defs>'.PHP_EOL;
