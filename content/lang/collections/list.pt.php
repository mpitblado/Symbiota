<?php
/*
------------------
Language: Português (Portuguese)
Translated by: Google Translate
Date Translated: 2024-02-16
------------------
*/

include_once('sharedterms.pt.php');

$LANG['PAGE_TITLE'] = 'Resultados da Pesquisa';
$LANG['NAV_SPECIMEN_LIST'] = 'Registros de Amostras';
$LANG['TABLE_DISPLAY'] = 'Exibição da Tabela';
$LANG['LIST_DISPLAY'] = 'Exibição da Lista';
$LANG['SHOW_ALL'] = 'mostrar tudo';
$LANG['DATASET'] = 'Conjunto de Dados';
$LANG['TAXA'] = 'Taxa';
$LANG['SEARCH_CRITERIA'] = 'Critérios de Pesquisa';
$LANG['PAGINATION_FIRST'] = 'Primeiro';
$LANG['ÚLTIMO'] = 'último';
$LANG['PAGINATION_PAGE'] = 'Página';
$LANG['PAGINATION_RECORDS'] = 'registros';
$LANG['PAGINATION_PREVIOUS'] = 'Anterior';
$LANG['PAGINATION_NEXT'] = 'Próximo';
$LANG['PAGINATION_OF'] = 'de';
$LANG['SELECT_DATASET'] = 'Por favor selecione um conjunto de dados para anexar ocorrências ou selecione Criar Novo Conjunto de Dados';
$LANG['SELECT_OCCURRENCE'] = 'Por favor selecione pelo menos uma ocorrência para ser adicionada ao conjunto de dados';
$LANG['DATASET_MANAGEMENT'] = 'Gerenciamento de Conjunto de Dados';
$LANG['DATASET_TARGET'] = 'Alvo do Conjunto de Dados';
$LANG['NO_DATASETS'] = 'nenhum conjunto de dados vinculado ao usuário';
$LANG['CREATE_DATASET'] = 'Criar Novo Conjunto de Dados';
$LANG['SELECT_ALL_RECORDS'] = 'Selecionar todos os registros da página';
$LANG['ADD_SELECTED'] = 'Adicionar Registros Selecionados ao Conjunto de Dados';
$LANG['ADD_COMPLETE_QUERY'] = 'Adicionar Consulta Completa ao Conjunto de Dados';
$LANG['OCCUR_EDIT_TITLE'] = 'Editar registro de ocorrência';
$LANG['VOUCHER_LINK_TITLE'] = 'Vincular voucher modelo a';
$LANG['PROTECTED'] = 'informações detalhadas da localidade protegidas';
$LANG['FULL_DETAILS'] = 'Detalhes completos do registro';
$LANG['NO_RESULTS'] = 'Sua consulta não retornou nenhum resultado. Por favor modifique seus parâmetros de consulta';
$LANG['PERHAPS_LOOKING_FOR'] = 'Talvez você estivesse procurando por:';
$LANG['MAP_DOWNLOAD'] = 'Baixar dados de coordenadas';
$LANG['MAP_HEADER'] = 'Mapa';
$LANG['MAP_DISPLAY'] = 'Exibir coordenadas no mapa';
$LANG['MAP_DESCRIPTION'] = 'O recurso de mapas fornece aos usuários um mapa 
        interativo que pode ser panorâmico (arrastando o mouse) e zoom (usando a roda do mouse). 
        Os pontos de coleta são exibidos como marcadores coloridos que, quando clicados, exibem as 
        informações completas daquela coleta. Quando múltiplas espécies são consultadas (separadas 
        por ponto e vírgula), marcadores coloridos diferentes indicam cada espécie individual.';
$LANG['KML_HEADER'] = 'KML';
$LANG['KML_DESCRIPTION'] = 'Isso cria um arquivo KML para uso no Google Earth ou qualquer outro aplicativo que use arquivos KML.';
$LANG['KML_EXTRA'] = 'Adicionar Campos Extras';
$LANG['CREATE_KML'] = 'Criar KML';
$LANG['DATASET_MANAGEMENT'] = 'Gerenciamento de Conjunto de Dados';
$LANG['IMG_DATASET_MANAGEMENT'] = 'Gerenciamento de Conjunto de Dados Aberto';
$LANG['IMG_TABLE_DISPLAY'] = 'Abrir Exibição da Tabela';
$LANG['IMG_DWNL_DATA'] = 'Botão de Download';
$LANG['IMG_COPY'] = 'Botão Copiar';
$LANG['ID_PROTECTED'] = 'Identificação Protegida';
$LANG['IMG_ADD_VOUCHER'] = 'Adicionar Voucher';
$LANG['IMG_EDIT_OCC'] = 'Editar Ocorrência';
$LANG['IMG_OCC'] = 'Imagem Associada à Ocorrência';
$LANG['CATALOG_NUMBER'] = 'Número de Catálogo';
$LANG['COLLECTOR'] = 'Colecionador';
$LANG['DATE'] = 'Data';
$LANG['SCIENTIFIC_NAME'] = 'Nome Científico';
$LANG['MAP_LINK'] = 'Link do Mapa';
$LANG['SEE_MAP_POINT'] = 'Ver ponto do mapa';
$LANG['RECORD_COUNT_EXCEEDS_LIMIT'] = 'A contagem de registros excede o limite';
$LANG['NO_RECORDS'] = 'Nenhum registro encontrado correspondente à consulta';
$LANG['SEARCH_RESULTS_LIST'] = 'Resultados da Pesquisa (Lista)';
?>
